...
function MinimaxTree({ boardMap, board }) {
  ...
  return (
    ...
    <button className="btn btn-primary" onClick={() => exportComponentAsPNG(treePdf, {fileName: "minimaxTree"})}>Generate PNG</button>
                ...
                <li>
                    ...
                    <ul>
                    {
                        boardMap[lastIndex].childs.map((board_child) => {
                            return <TreeChild key={Math.random().toString()} boardMap={board_child}/>
                        })
                    }
                    </ul>
                </li>
